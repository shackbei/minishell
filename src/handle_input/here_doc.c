...
/*   Updated: 2021/12/07 22:12:42 by shackbei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	read_here_doc(t_data *data, char *limiter, int fd)
{
	...
	while (1)
	{
		...
		if (ft_strncmp(limiter, inpt, ft_strlen(limiter)) == 0)
		{
			data->exit_val = 0;
			break ;
		}
...
static void	ft_get_delimiter(t_temp	*temp, t_data *data)
{
	...
	if (!limiter)
		return ;
	data->here_doc_qot = 0;
	if (ft_strchr(limiter, '\'') != -1 || ft_strchr(limiter, '\"') != -1)
		data->here_doc_qot = 1;
	limiterqots = limiter;
	ft_indent_qots(data, &limiter, 0);
	here_doc(data, limiter, ft_getfilename(data, limiterqots, &filename));
	i = -1;
	while (filename[++i])
